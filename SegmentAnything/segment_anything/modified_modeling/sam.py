...
class MySam(nn.Module):
    ...
    @property
    def device(self) -> str:
        assert self.image_encoder.device == self.prompt_encoder.device == self.mask_decoder.device
        return self.image_encoder.device
    
    @property
    def all_required_grad(self) -> bool:
    ...
    def forward(
        ...
        print('[input_images shape in MySam: {}]'.format(input_images.shape))
        ...
        for curr_embedding in image_embeddings:
            ...
            logits_output = self.postprocess_masks(
                low_res_masks,
                input_size=input_images.shape[-2:],
                original_size=input_images.shape[-2:],
            )
        ...
        return torch.cat(logits_outputs, dim=0)
